...
jQuery(document).ready(function($) {
  $('.nav-item_link').on('click', function() {
    if(!$(this).hasClass('nav-item_link--bordered')) {
  ...
  scrollIn($('.fade-in, .slide-in'), 150, 'active');
  $(window).on('scroll', function () {
    scrollIn($('.fade-in, .slide-in'), 150, 'active');
  });
  $(".ajax-form").submit(function(event) { //устанавливаем событие отправки для формы с id=form
    event.preventDefault();
    const form = $(this);
    ...
    $.ajax({
      ...
      success: function() {
        ...
        setTimeout(() => {
          form.find('.form-message').fadeIn(), 600
        }, 600);
      }
    });
  });
